...
// RedpackModel 搖紅包的資料表欄位
type RedpackModel struct {
	Base `json:"-"`
...
// DefaultRedpackModel 預設RedpackModel
func DefaultRedpackModel() RedpackModel {
	return RedpackModel{Base: Base{TableName: "activity_set_redpack"}}
}

// GetRedpackModelAndID 設置RedpackModel與ID
func GetRedpackModelAndID(tablename, id string) RedpackModel {
	idInt, _ := strconv.Atoi(id)
	return RedpackModel{Base: Base{TableName: tablename}, ID: int64(idInt)}
}

// SetConn 設定connection
func (m RedpackModel) SetConn(conn db.Connection) RedpackModel {
	m.Conn = conn
	return m
}

// SetTx 設置Tx
func (m RedpackModel) SetTx(tx *dbsql.Tx) RedpackModel {
	m.Base.Tx = tx
	return m
}

// AddRedpack 增加搖紅包獎品
func (m RedpackModel) AddRedpack(activityid, gameid, title, repeat,
	status string, second, percent int) (RedpackModel, error) {
	// 檢查是否有該活動
...
// UpdateRedpack 更新搖紅包資料
func (m RedpackModel) UpdateRedpack(activityid, gameid, title, repeat,
