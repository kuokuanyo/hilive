...
func GetManagerFormPanel(conn db.Connection) (managerTable Table) {
	// 建立BaseTable
	...
	formList.SetInsertFunc(func(values form2.Values) error {
		...
		_, txErr := db.SetConnectionAndCRUD(conn).WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
			// 新增用戶資料
			user, err := models.DefaultUserModel("users").SetTx(tx).SetConn(conn).AddUser(values.Get("userid"), values.Get("username"),
				values.Get("phone"), values.Get("email"), password)
			if err != nil {
				if err.Error() != "沒有影響任何資料" {
					return errors.New("新增用戶發生錯誤"), nil
				}
			}
			// 新增角色、權限
			for i := 0; i < len(values["role_id[]"]); i++ {
