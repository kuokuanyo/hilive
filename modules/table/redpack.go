...
// GetRedpackPanel 取得搖紅包頁面、表單資訊
func (s *SystemTable) GetRedpackPanel(ctx *context.Context) (gameTable Table) {
	// 建立BaseTable
	...
	info.SetTable("activity_set_redpack").SetTitle("搖紅包").SetDescription("遊戲管理").
		SetDeleteFunc(func(idArr []string) error {
			var ids = interfaces(idArr)

			_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
				err := s.connection().SetTx(tx).
					Table("activity_set_redpack").WhereIn("id", ids).Delete()
				if err != nil {
	...
	formList.AddField("允許重複搖中", "allow_repeat", db.Tinyint, form.Radio).
		...
		SetDisplayFunc(func(value types.FieldModel) interface{} {
			...
			statusModel, _ := s.table("activity_set_redpack").Select("allow_repeat").FindByID(value.ID)
	...
	formList.AddField("遊戲狀態", "game_status", db.Tinyint, form.Radio).
		...
		SetDisplayFunc(func(value types.FieldModel) interface{} {
			...
			statusModel, _ := s.table("activity_set_redpack").Select("game_status").FindByID(value.ID)
			stauts = append(stauts, strconv.FormatInt(statusModel["game_status"].(int64), 10))
			return stauts
		}).SetFieldDefault("1")

	formList.SetTable("activity_set_redpack").SetTitle("搖紅包").SetDescription("遊戲管理")
	...
	formList.SetInsertFunc(func(values form2.Values) error {
		...
		_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
			_, err := models.DefaultRedpackModel().SetTx(tx).SetConn(s.conn).AddRedpack(
	...
	formList.SetUpdateFunc(func(values form2.Values) error {
		...
		second, _ := strconv.Atoi(values.Get("second"))
		percent, _ := strconv.Atoi(values.Get("percent"))
		model := models.GetRedpackModelAndID("activity_set_redpack", values.Get("id")).SetConn(s.conn)
		_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
			_, err := model.SetTx(tx).UpdateRedpack(values.Get("activity_id"), values.Get("game_id"),
