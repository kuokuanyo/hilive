...
// GetRedpackPrizePanel 取得搖紅包獎品頁面、表單資訊
func (s *SystemTable) GetRedpackPrizePanel(ctx *context.Context) (prizeTable Table) {
	// 建立BaseTable
	...
	info.SetTable("activity_redpack_prize").SetTitle("搖紅包").SetDescription("獎品管理").
		SetDeleteFunc(func(idArr []string) error {
			var ids = interfaces(idArr)

			_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
				err := s.connection().SetTx(tx).
					Table("activity_redpack_prize").WhereIn("id", ids).Delete()
				if err != nil {
	...
	formList.AddField("獎品類型", "prize_type", db.Tinyint, form.Radio).
		...
		SetDisplayFunc(func(value types.FieldModel) interface{} {
			...
			statusModel, _ := s.table("activity_redpack_prize").
	...
	formList.SetTable("activity_redpack_prize").SetTitle("搖紅包").SetDescription("獎品管理")
	...
	formList.SetInsertFunc(func(values form2.Values) error {
		...
		_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
			_, err := models.DefaultRedpackPrizeModel().SetTx(tx).SetConn(s.conn).AddRedpackPrize(
	...
	formList.SetUpdateFunc(func(values form2.Values) error {
		...
		model := models.GetRedpackPrizeModelAndID("activity_redpack_prize", values.Get("id")).SetConn(s.conn)
		_, txErr := s.connection().WithTransaction(func(tx *sql.Tx) (e error, i map[string]interface{}) {
			_, err := model.SetTx(tx).UpdateRedpackPrize(values.Get("game_id"), values.Get("prize_name"), values.Get("prize_type"),
